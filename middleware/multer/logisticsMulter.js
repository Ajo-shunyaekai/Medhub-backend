...
const addLogisticsUpload = (req, res, next) => {
  upload.fields([{ name: "pod", maxCount: 4 }])(req, res, async (err) => {
    if (err) {
    ...
    const getUploadedFilesPath = async () => {
      if (req?.files?.["pod"]) {
        uploadedFiles["pod"] = await uploadMultipleFiles(
          // req?.files?.["pod"] || []
          (req?.files?.["pod"] || [])?.map((file) => ({
            ...file,
            path: file.path,
      ...
      removeLocalFiles([...(req?.files?.["pod"] || [])]);
...
const editLogisticsUpload = (req, res, next) => {
  ...
  uploadEdit.fields([{ name: "podNew", maxCount: getMaxCount("pod") }])(
    req,
    res,
    async (err) => {
      if (err) {
      ...
      const getUploadedFilesPath = async () => {
        if (req?.files?.["podNew"]) {
          uploadedFiles["podNew"] = await uploadMultipleFiles(
            // req?.files?.["podNew"] || []
            (req?.files?.["podNew"] || [])?.map((file) => ({
              ...file,
              path: file.path,
              filename: file.filename,
              contentType: file.mimetype,
        ...
        // Remove uploaded files from local storage
        removeLocalFiles([...(req?.files?.["podNew"] || [])]);
