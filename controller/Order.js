...
module.exports = {
  createOrder: async (req, res, reqObj, callback) => {
    try {
      ...
      const newOrder = new Order({
        ...
        buyer_email: buyer.contact_person_email,
  ...
  bookLogistics: async (req, res, reqObj, callback) => {
    try {
      ...
      const recipientEmails = [supplier.contact_person_email];
      const emailContent = await bookLogisticsContent(
        supplier,
        buyer,
        order_id
      );
        ...
        data = await Order.aggregate([
          ...
          },
          {
            $group: {
              ...
              buyer_email: { $first: "$buyer.contact_person_email" },
              buyer_address: { $first: "$buyer_address" },
              buyer_mobile: { $first: "$buyer_mobile" },
              supplier_name: { $first: "$supplier_name" },
              supplier_email: { $first: "$supplier.contact_person_email" },
          ...
          },
          {
            $project: {
              ...
              supplier_registered_address: {
                company_reg_address:
                  "$supplier.registeredAddress.company_reg_address",
