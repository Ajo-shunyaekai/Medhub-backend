    ...
    try {
      ...
      enquiry.quotation_items.forEach((detail) => {
        if (
          itemIds.some(
            (itemId) =>
              detail._id.equals(new ObjectId(itemId)) &&
      ...
      enquiry.quotation_items.forEach((detail) => {
        if (
          rejectedIds.some(
            (rejectedItemId) =>
              detail._id.equals(new ObjectId(rejectedItemId)) &&
      ...
      rejectedItems.forEach((rejectedItem) => {
        const enquiryItem = enquiry.items.find(
          (item) => item.product_id === rejectedItem.product_id
        );
        if (enquiryItem) {
  ...
  getPOList: async (req, res, reqObj, callback) => {
    try {
      ...
      if (usertype === "Admin") {
        ...
        if (!supplier_id) {
          (query.buyer_id = buyer_id), (query.po_status = status);
        } else if (!buyer_id) {
          (query.supplier_id = supplier_id), (query.po_status = status);
        }
          ...
          },
          {
            $addFields: {
              ...
              buyer_registered_address: {
                ...
                locality: {
                  $arrayElemAt: [
                    "$buyer_details.registeredAddress.locality",
                    0,
                  ],
                },
                land_mark: {
                  $arrayElemAt: [
                    "$buyer_details.registeredAddress.land_mark",
                    0,
                  ],
                },
                city: {
              ...
              supplier_registered_address: {
                ...
                state: {
                  $arrayElemAt: [
                    "$supplier_details.registeredAddress.state",
                    0,
                  ],
                },
                country: {
  ...
  editPO: async (req, res, reqObj, callback) => {
    try {
      ...
      if (
        ...
        purchaseOrder.enquiry_id !== enquiry_id
      ) {
        return callback({
          code: 403,
      ...
      purchaseOrder.supplier_address =
        supplierAddress || purchaseOrder.supplier_address;
      purchaseOrder.supplier_email =
        supplierEmail || purchaseOrder.supplier_email;
      purchaseOrder.supplier_mobile =
        supplierMobile || purchaseOrder.supplier_mobile;
      purchaseOrder.supplier_country_code =
        supplier_country_code || purchaseOrder.supplier_country_code;
      purchaseOrder.supplier_regNo =
        supplierRegNo || purchaseOrder.supplier_regNo;
      ...
      purchaseOrder.buyer_country_code =
        buyer_country_code || purchaseOrder.buyer_country_code;
      purchaseOrder.buyer_regNo = buyerRegNo || purchaseOrder.buyer_regNo;
      purchaseOrder.additional_instructions =
        description || purchaseOrder.additional_instructions;
