  ...
  try {
    ...
    if (!user) {
      return res
        ?.status(400)
        ?.send({
          message: error?.message || "Failed finding user details for address",
          result: {},
        });
    }
    ...
    if (!address) {
      return res
        ?.status(400)
        ?.send({
          message: error?.message || "Failed finding address details",
          result: {},
        });
    }

    const updatedAddress = [
      user?.buyer_address || user?.supplier_address,
      ...address,
    ]?.filter((ele) => ele != undefined || ele != null);

    return res
      ?.status(200)
      ?.send({ message: "Success get Address!", address: updatedAddress });
  } catch (error) {
...
const addAddress = async (req, res) => {
  try {
    ...
    if (!address) {
      return res
        ?.status(400)
        ?.send({ message: "Failed finding address details", result: {} });
    }
    ...
    return res
      ?.status(200)
      ?.send({ message: "Success add Address!", address: updatedAddress });
  } catch (error) {
    console.log("Internal Server Error:", error);
    logErrorToFile(error, req);
    return sendErrorResponse(res, 500, "An unexpected error occurred. Please try again later.", error);
...
const deleteAddress = async (req, res) => {
  try {
    ...
    return res
      ?.status(200)
      ?.send({ message: "Success delete Address!", address: updatedAddress });
  } catch (error) {
    console.log("Internal Server Error:", error);
    logErrorToFile(error, req);
    return sendErrorResponse(res, 500, "An unexpected error occurred. Please try again later.", error);
