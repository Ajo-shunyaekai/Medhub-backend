...
const getAllBids3 = async (req, res) => {
  try {
    ...
    const matchStage = {
      ...(userId && { userId }),
      // ...(status && userType?.toLowerCase() == "buyer" { status }),
      ...(userType === "Supplier" &&
        country && { "general.fromCountries": country }),
      ...(categoryArray.length > 0 && {
        additionalDetails: {
    ...
    const filteredBidsWithStatus = await Promise.all(
      finalBids.map(async (bid) => {
        const { startDate, startTime, endDate, endTime, country } =
          bid.general || {};
        ...
        // Include only bids that match the status
        const isMatch = bidStatus?.toString()?.toLowerCase() === status?.toLowerCase();
        return isMatch ? bid : null;
      })
    );

    // Filter out nulls
    finalBids = filteredBidsWithStatus.filter((bid) => bid !== null);

    if (status !== "completed") {
      finalBids = finalBids
        ?.map((bid) => {
          const allQuoted = bid.additionalDetails.every(
            (item) => item?.quoteRequested
          );

          // If not all are quoted, return the original bid
          if (!allQuoted) {
            return bid;
          }

          // Optionally, return something else if allQuoted === true
          // For example, return null or keep the original bid
          return null; // or null or modify as needed
        })
        .filter((bid) => bid !== null);
    }
    // }
...
const addToFavourite = async (req, res) => {
  try {
    ...
    const updated = await bidDetails.save();

    if (!updated) {
      return sendErrorResponse(res, 404, "Error updating status");
    }
