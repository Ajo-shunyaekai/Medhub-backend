...
module.exports = {
    ...
    try {
      ...
      if (typeof stockedInDetails === "string") {
        try {
          ...
          return callback({
            code: 400,
            message: "Invalid format for stocked_in_details",
          });
        }
      }
      ...
      if (![quantity, unit_price, est_delivery_days].every(Array.isArray)) {
        return callback({
          code: 400,
          message: "Inventory fields should be arrays",
        });
      }

      if (
        quantity.length !== unit_price.length ||
        unit_price.length !== est_delivery_days.length
      ) {
        return callback({
          code: 400,
          message: "All inventory arrays must have the same length",
        });
      }
      ...
      const event =
        product_type === "new"
          ...
          : "addsecondarymedicinerequest";
      const newNotification = new Notification({
        notification_id: notificationId,
        event_type:
          product_type === "new"
            ? "New Medicine Request"
            : "New Secondary Medicine Request",
        event,
        from: "supplier",
        to: "admin",
        from_id: supplier_id,
        event_id: product_id,
        message: `${
          product_type === "new" ? "New medicine" : "New secondary medicine"
        } request from ${supplier_id}`,
        status: 0,
      });
      ...
      callback({
        code: 200,
        message: "Add Medicine Request Submitted Successfully",
  ...
  allMedicineList: async (req, res, reqObj, callback) => {
    try {
      ...
      if (in_stock && in_stock.length > 0) {
        const stockedCountries = in_stock[0]
          .split(",")
      ...
      if (delivery_time && delivery_time.length > 0) {
        ...
        const deliveryConditions = ranges.map((range) => {
          if (range.includes("greater than")) {
            ...
            return {
              "inventory_info.est_delivery_days": { $gt: value.toString() },
            };
          } else {
            const [min, max] = range
              .split("-")
            ...
            // };
            return {
              "inventory_info.est_delivery_days": {
                $gte: min.toString(),
                $lte: max.toString(),
              },
            };
      ...
      if (product_type === "new") {
        ...
        newMedEdit
          ...
          .then(async (savedMedicine) => {
            ...
            const event =
              product_type === "new"
                ? "editnewmedicinerequest"
                : "editsecondarymedicinerequest";
            const newNotification = new Notification({
              notification_id: notificationId,
  ...
  medicineEditList: async (req, res, reqObj, callback) => {
    try {
      ...
      EditMedicine.find({ edit_status: status, supplier_id: supplier_id })
        .sort({ createdAt: -1 })
        .skip(offset)
        .limit(page_size)
        .then((data) => {
        ...
        .catch((err) => {
          callback({
            code: 400,
            message: "Error while fetching medicine edit list",
            result: err,
          });
        });
    } catch (error) {
  ...
  filterMedicine: async (req, res, reqObj, callback) => {
    try {
      ...
      Medicine.aggregate(aggregatePipeline)
        .then((result) => {
          callback({
            code: 200,
            message: `Filtered Medicine lists`,
            result: result,
          });
        })
        .catch((err) => {
        ...
        },
        {
          $addFields: {
            "inventory_info.unit_price_numeric_str": {
              $regexFind: {
                input: "$inventory_info.unit_price",
                regex: "[0-9.]+",
              },
            },
            "inventory_info.est_delivery_days_numeric_str": {
              $regexFind: {
                input: "$inventory_info.est_delivery_days",
                regex: "[0-9]+",
              },
            },
          },
        ...
        const deliveryConditions = ranges.map((range) => {
          if (range.includes("greater than")) {
            ...
            const [min, max] = range
              ...
              .map((num) => parseInt(num.trim()));
            return {
              "inventory_info.est_delivery_days_num": { $gte: min, $lt: max },
            };
          }
        });
        pipeline.push({ $match: { $or: deliveryConditions } });
        ...
        .then((data) => {
          Medicine.countDocuments({
            ...
            .catch((err) => {
              callback({
                code: 400,
                message: "Error while fetching similar medicine list count",
                result: err,
              });
            });
        })
  ...
  getSpecificMedicinesList: async (req, res) => {
    try {
      ...
      let matchCondition = {
        medicine_type:
          medicine_type?.replaceAll("%20", " ") ||
          medicineType?.replaceAll("%20", " "),
      };
      ...
      const totalItems = await Medicine.countDocuments(
        usertype == "Admin"
          ...
          : { medicine_type: medicine_type?.replaceAll("%20", " ") }
      );
