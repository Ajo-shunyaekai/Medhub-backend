...
const { sendSuccessResponse } = require("../utils/commonResonse");
...
const markExpiredOrFullyQuotedBidsAsCompleted2 = async (req, res) => {
  try {
    console.log("\n\nfunction called");
    ...
    for (const bid of bidsToUpdate) {
      ...
      if (!startDate || !startTime || !endDate || !endTime || !country) {
        console.warn(
          `Skipping bid ${bid.bid_id}: Missing required general fields`
        );
        continue;
      }
      ...
      try {
        ...
        console.warn(
          `Timezone comparison failed for bid ${bid.bid_id}:`,
          err.message
        );
        continue; // skip this bid if timezone conversion fails
      }
      ...
      if (bidStatus === "completed" || allQuoteRequested) {
        ...
        await Bid.updateOne({ _id: bid._id }, { $set: { status: bidStatus } });
    ...
    console.error("❌ Error in markExpiredBidsAsCompleted2:", error);
...
const scheduleExpiredBidsCronJob = () => {
  cron.schedule("*/2 * * * *", async () => {
    await markExpiredOrFullyQuotedBidsAsCompleted2();
  });
};
//bid expiry
  ...
  try {
    ...
    for (const bid of activeBids) {
      ...
      if (now >= startDateTime) {
        ...
        const openFor =
          bid.additionalDetails?.map((s) => s?.openFor).filter(Boolean) || [];
        const categories =
          bid.additionalDetails?.map((s) => s?.category).filter(Boolean) || [];
        ...
        await Bid.updateOne({ _id: bid._id }, { $set: { notified: true } });
...
const markExpiredSubscriptionsAsExpired = async () => {
  ...
  try {
    ...
    for (const sub of allSubscriptions) {
      ...
      const reminderDays =
        sub.productName === "Monthly Subscription" ? [3, 1] : [7, 3, 1];
      ...
      if (shouldSendReminder || isExpired) {
        ...
        const emailContent = await sendSubscriptionExpiryEmailContent(
          userType,
          user,
          sub,
          diffInDays
...
const scheduleExpiredSubscriptionsCronJob = () => {
  //runs every morning 8am
