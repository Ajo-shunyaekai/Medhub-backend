...
const markExpiredOrFullyQuotedBidsAsCompleted2 = async (req, res) => {
  try {
    const bidsToUpdate = await Bid.find({ status: { $in: ["active", "inactive"] } });
    let updatedCount = 0;
...
const scheduleExpiredBidsCronJob = () => {
  cron.schedule("*/2 * * * *", async () => {
    ...
    await markExpiredOrFullyQuotedBidsAsCompleted2(); // 👈 updated function
  });
};
//bid expiry
...
const markExpiredSubscriptionsAsExpired = async () => {
  ...
  try {
    ...
    // console.log(`Removed currentSubscription from ${updatedCount} user(s).`);
  } catch (error) {
